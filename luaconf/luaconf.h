...
#define LUA_FLOAT_INTEGER	4 //TOMSCI
...
#elif LUA_FLOAT_TYPE == LUA_FLOAT_INTEGER	/* }{ tomsci - as per LUA_INTEGER */

#define LUA_NUMBER		LUA_INTEGER

#define l_mathlim(n)		(_TOMSCI_dummy_ ## n)
...
//" TOMSCI fixup for sublime formatter
...
#define lua_number2strx(L, buff, sz, fmt, x) (sprintf(buff, fmt[2]=='A'?"%lX":"%lx", x))

static inline LUA_NUMBER luai_numidiv(void* L, LUA_NUMBER a, LUA_NUMBER b) {
	(void)L;
	// This is tedious...
	if (a < 0 && b > 0) {
		return (a - b + 1) / b;
...
static inline long luai_nummod_helper(void* L, LUA_NUMBER a, LUA_NUMBER b) {
	LUA_NUMBER numerator;
	if (a < 0) numerator = a - (b - 1); // Hack for integer division rounding towards zero not -inf
	else numerator = a;
	return a - (numerator/b)*b;
}
#define luai_nummod(L,a,b,m) (m) = luai_nummod_helper(L, (a), (b))

// 10^9 is the largest exponent which fits in our (32-bit) LUA_NUMBER
#define _TOMSCI_dummy_MAX_10_EXP (9)

#define l_randomizePivot() (~0)

#if defined(LUACONF_FULL_FAT_STDIO) //TOMSCI

// lauxlib.h now defines the stdio lua_writestring/lua_writeline/lua_writestringerror
...
#define lua_writestring(s,l) lupi_printstring(s)
#define lua_writeline() lupi_printstring("\n")
#define lua_writestringerror(s, p) lupi_printstring(s); lupi_printstring(p); lua_writeline() //TODO the format parameter!
